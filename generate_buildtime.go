...
var buildtimeTmpl = template.Must(template.New("").Parse(`package server
...
var (
	faviconRegexp = regexp.MustCompile({{ .favicon }})
...
func main() {
	f, err := os.Create("./server/buildtime.go")
	check(err)
	defer f.Close()

	err = buildtimeTmpl.Execute(f, map[string]interface{}{
		"buildtime": time.Now().Unix(),
		"favicon":   "`<link href=\"([^\"]*)\" rel=\"icon\">`",
